...
async def add_image(message: Message, state:FSMContext):
    ...
    await message.answer('Введите ключ игры')
...
async def delete_categories(message: Message):
    if not await check_admin(message):
        await message.answer("Это команда только для администратора :(")
        return
    await message.answer('Выберите категорию для удаления', reply_markup=await categories_kb3())
...
async def del_game(callback: CallbackQuery):
    ...
    #     await callback.message.answer("Это команда только для администратора :(")
    #     return
